...
class CRC32{
	...
	public static func CalcCrc32(crcInit:UInt32, length:Int,byte:[UInt8], offset:Int)->UInt32{
	var crc = crcInit
	var i = offset
	var len = length
	while (len > 0){
		crc = self.m_crcTable[((Int(byte[i]) ^ Int (crc)) & 0xff)] ^ (crc >> 8)
		i += 1
		len -= len
	}
