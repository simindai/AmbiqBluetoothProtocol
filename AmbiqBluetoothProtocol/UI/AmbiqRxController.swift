...
class AmbiqRxController: UIViewController,MenuViewDelegate{
	...
	func add_controls(){
		...
		self.m_titleLabel = Common.createlabel("Ambiq Rx characteristic", font:self.m_constraint.titleLableFont , color: constantsFontColor.setupLabelColor, autoSizeConstraints: false)
		...
		self.m_testLabel = Common.createlabel("Send Test Data", font:self.m_constraint.buttonLabelFont , color: constantsFontColor.setupLabelColor, autoSizeConstraints: false)
		self.m_rxActionSwitch = UISwitch()
		self.m_rxActionSwitch.translatesAutoresizingMaskIntoConstraints = false
		self.m_rxActionSwitch.addTarget(self, action: #selector(self.action_switch(_:)), for: .valueChanged)
		...
		self.m_actionView.addSubview(self.m_rxActionSwitch)
	...
	func setup_action_view(){
		...
		self.m_rxActionSwitch.rightAnchor.constraint(equalTo: magin_guide.rightAnchor).isActive = true
		self.m_rxActionSwitch.centerYAnchor.constraint(equalTo: self.m_testLabel.centerYAnchor).isActive = true
	}
	...
	@objc func action_switch(_ send:UISwitch){
		if send.isOn{
			//print("on")
		    self.m_delegate.sendTestDataToServer(p: self.m_peripheral, rxCharacteristic: self.m_characteristic)
		}
		else  {
			//print("off")
			clean_up()
