...
class ServiceTitleView:UIView{
	...
	var m_service_title_view_delegate:ServiceTitleViewDelegate!
	var m_peripheral:CBPeripheral!
	
	var m_status = Common.CONNECTED
	override init(frame: CGRect) {
        super.init(frame: frame)
    }
	
	init(delgate:ServiceDelegate, serviceTitleViewDelegate:ServiceTitleViewDelegate, p:CBPeripheral){
	  super.init(frame:CGRect())
	  self.setup()
	  self.m_service_delegate = delgate
	  self.m_service_title_view_delegate = serviceTitleViewDelegate
	   self.m_peripheral  = p
	}
	...
	{
		static let setupLabelFontIphone = UIFont(name:"HelveticaNeue-Bold", size: 20.0)!
	...
	func setup(){
		...
		NotificationCenter.default.addObserver(self, selector: #selector(self.connectAction(_:)), name: .didDiscoverService, object: nil)
	...
	private func add_controls(){
		self.m_label = Common.createlabel("Services", font: self.m_constraint.titleLabelFont, color: UIColor.white, autoSizeConstraints: false)
		self.m_label.backgroundColor = constantsFontColor.setupLabelColor
		self.m_label.textAlignment = .center
		self.m_label.sizeToFit()
		self.m_backButton = Common.createButton("<", font: self.m_constraint.buttonFont, tcolor: UIColor.white, bcolor: constantsFontColor.buttonColor, alpha: 1.0, autoSizeConstraints: false)
	...
	private func set_constraints(){
		...
		//self.m_backButton.widthAnchor.constraint(equalTo: marginGuide.widthAnchor, multiplier: self.m_constraint.backButtonWidthFrac).isActive = true
		
		//self.m_label.leftAnchor.constraint(equalTo: self.m_backButton.rightAnchor).isActive = true
		...
		//self.m_connectButton.widthAnchor.constraint(equalTo: marginGuide.widthAnchor,multiplier: self.m_constraint.connectButtonWidthFrac).isActive = true
	}
	...
	@objc func backAction(_ sender: UIButton){
		self.m_service_title_view_delegate.service_release()
	}
	...
	@objc func disconnectAction(_ notification: Notification){
		DispatchQueue.main.async {
			self.m_status = Common.DISCONNECTED
	...
	@objc func connectAction(_ notification: Notification){
		DispatchQueue.main.async {
			self.m_status = Common.CONNECTED
		    self.m_connectButton.setTitle("Disconnect", for: .normal)
			self.m_label.text = "Services"
